...
function detectarColumnasRepetibles(array $rows, int $maxDistinct = 12, int $minRepeat = 2): array {
    ...
    foreach($cols as $col){
        ...
        // Heurística: si parece numérica y con mucha variedad, no es buena para tarta.
        // (permitimos numéricos si la cardinalidad es baja)
        $allNumeric = true;
        foreach(array_keys($counts) as $k){
            $k2 = str_replace([',',' '], ['.',''], $k);
            if($k === "(NULL)" || $k === "(vacío)") { $allNumeric = false; break; }
            if(!is_numeric($k2)) { $allNumeric = false; break; }
        }
    ...
    // ordena candidatas por "potencial": más filas en top1 y menos distintos
